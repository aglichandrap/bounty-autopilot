...
- `degraded` Bounty scout scheduler: bounty_worker_queue.md is stale (184 minutes old); dispatch sent.
- `degraded` GitHub claimer scheduler: github_bounty_claim_report.md is stale (267 minutes old); dispatch sent.
- `degraded` GitHub submitter scheduler: github_bounty_submission_report.md is stale (261 minutes old); dispatch sent.
- `missing` TaskBounty scout scheduler: taskbounty_scout_report.md has no parseable timestamp; dispatch sent.
- `degraded` TaskBounty worker scheduler: taskbounty_worker_report.md is stale (263 minutes old); dispatch sent.
