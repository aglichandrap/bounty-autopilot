...
- `ok` Bounty scout scheduler: bounty_worker_queue.md is fresh (18 minutes old).
- `degraded` GitHub claimer scheduler: github_bounty_claim_report.md is stale (95 minutes old); dispatch sent.
- `degraded` GitHub submitter scheduler: github_bounty_submission_report.md is stale (94 minutes old); dispatch sent.
- `degraded` TaskBounty scout scheduler: taskbounty_report.md is stale (101 minutes old); dispatch sent.
- `degraded` TaskBounty worker scheduler: taskbounty_worker_report.md is stale (101 minutes old); dispatch sent.
